...
#endif`;function yn(n,e,o){return Object.fromEntries(Object.entries(e).map(([t,r])=>[t,{param:r,location:`u_${n()}_${t}`,signal:o[t]??m(r[1])}]))}function C(n,e,o){return(t,r)=>{const i=yn(n,e,r);return o(t,i)}}let mn=0;function p(){return`n${mn++}`}const pn=C(p,{dimensions:["vec3",{x:1,y:1,z:1}]},({},n)=>e=>{const o=p();return[o,[{defs:[`float sdf_box(vec3 p, vec3 b) {
vec3 q = abs(p) - b;
...
void main() {
  uv = a_uv;
  gl_Position = a_position;
}`,[v,_]=W(t,d,s),w=t.getAttribLocation(v,"a_position"),E=t.getAttribLocation(v,"a_uv"),R={};for(let g of f)R[g]=t.getUniformLocation(v,g);const S=t.getUniformLocation(v,"u_maxIterations"),h=t.getUniformLocation(v,"u_fresnelBase"),P=t.getUniformLocation(v,"u_fresnelExp"),L=t.getUniformLocation(v,"u_mixFactor");function B(g){t.useProgram(v);let c=0;for(let y of x)for(let b of Object.values(y.params))c=Q(t,b.param[0],R[b.location],b.signal.value(g),c)}const[N,O,I]=j(n.gl,a);return{render:k(g=>{O(),B(g);const c=r(g);t.uniform1i(S,c.maxIterations),t.uniform1f(h,c.fresnelBase),t.uniform1f(P,c.fresnelExp),t.uniform1f(L,c.mixFactor),H(n,{a_position:w,a_uv:E})}),out:N,destroy:()=>{I(),_()}}}async function bn(n,e,o,t){const r=F(123455,1,100),i=F(123455,5,10),a=F(123455,.5,100),s=F(1348347,.05,2),l=F(148347,5,100),x=await sn(),u={x:.25,y:.25},f=o.zip(t).zip(x).map(([[c,y],b])=>.5+(.5-y.x/c)+b.y/60+b.z/60),d=f.map(c=>({x:u.x,y:u.y,z:c}));async function*v(){await A(2e3),yield a.zip(i).zip(f).map(([[c,y],b])=>({x:c,y,z:b})),await A(2e3),yield d,yield*v()}const _=pn({},{dimensions:Y(d,v)}),w=zn(_n(_,{axis:m({x:1,y:1,z:1}),radians:l.map(Math.floor)}),{k:m(.5)});async function*E(){yield M.map(c=>({x:(c-Math.floor(c))/2,y:0,z:0})),await A(3500),yield M.map(c=>({x:0,y:-(c-Math.floor(c))/2,z:0})),await A(2500),yield M.map(c=>({x:-(c-Math.floor(c))/2,y:0,z:0})),await A(2500),yield*E()}const R=X($({},{radius:s.map(c=>c+.25)}),{translate:Y(m({x:0,y:0,z:0}),E)}),S=X($({},{radius:m(.25)}),{translate:m({x:0,y:.4,z:0})}),h=G({sdf1:G({sdf1:w,sdf2:R},{}),sdf2:S},{k:r});async function*P(){await A(1e3),yield l.map(c=>c+1),await A(3e3),yield m(10),yield*P()}const L=Pn(e,{...n,sdf:h},{maxIterations:Y(m(10),P),mixFactor:m(.8)}),B=un(e,{...n,tex:m(L)},{offset:m({x:.3,y:.3})}),N=ln(e,{...n,tex:m(B)},{t:M,multiplier:m(25)}),{render:O}=dn(e,N.out,0,0,1);function I(c){const y=e.gl;N.render(c),y.bindFramebuffer(y.FRAMEBUFFER,null),y.viewport(0,0,y.canvas.width,y.canvas.height),y.clearColor(0,0,0,0),y.clear(y.COLOR_BUFFER_BIT),O()}let D=0,U=!1;function g(c){I(D),D++,U||requestAnimationFrame(()=>g())}return{run:()=>g(),cancel:()=>{U=!0}}}function hn(){const[n,e]=rn(0),o=vn(),t=document.getElementById("canvas"),r=document.getElementById("pretentious"),i=document.getElementById("narrative"),a=document.getElementById("enter"),s=document.getElementById("future"),l=an(document.body);let x=()=>{};async function u(){t.width=window.innerWidth,t.height=window.innerHeight-15,e(window.innerWidth);const d={width:t.width,height:t.height,format:"byte"};let v=await bn(d,o,n,l);x(),v.run(),x=v.cancel}let f=0;a.addEventListener("click",d=>{var v,_;f==0?(a.innerHTML="[ Onwards ]",(v=s.parentElement)==null||v.removeChild(s),i.innerHTML=`
